...
async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Only include max_participants if the user provided a value
      if (form.max_participants && form.max_participants.trim() !== '') {
        payload.max_participants = parseInt(form.max_participants, 10);
      }

  const created: any = await eventsApi.create(payload);
  // eventsApi unwraps res.data, so `created` is the created object
